package com.criticalfusion.training.player.controllers
{
	...
	import com.criticalfusion.training.player.model.TextItems;
	...
	public class LessonManager extends EventDispatcher
	{
		...
		protected static var _instance:LessonManager;
		
		protected var XMLFile:String = "lessons.xml";
		protected var _menuList:Array;
		protected var _text:TextItems;
		...
		public static function get instance():LessonManager 
		{
			if(LessonManager._instance == null)  LessonManager._instance = new LessonManager(new SingletonEnforcer());
		...
		protected function onLoadedMenu(e:Event):void 
		{
			XML.ignoreWhitespace = true;
			var xml:XML = new XML(e.target.data);
			parseCategories(xml..category);
			//parseLessons(xml.children());
		}
		
		protected function parseCategories(xml:XMLList):void
		{
			var i:uint;
			var c:Category;
			for(i = 0; i < xml.length(); i++)
			{
				...
				c.id = xml[i].@id;
				c.file = xml[i].@file;
				parseLessons(xml[i].children(), c);
				categories.push(c);
			}
			dispatchEvent(new Event(LessonManager.LESSONS_READY));
		}
		
		protected function parseLessons(xml:XMLList, category:Category):void 
		{
			for( var i:int = 0; i < xml.length(); i++) 
			{
				var lesson:Lesson = new Lesson();
				lesson.id = Number(xml[i].@id.toString());
				lesson.label = xml[i].@title.toString();
				lesson.subsection = (xml[i].@subsection == "true");
				lesson.scene = xml[i].@scene;
				
				addTextToList(xml[i]..text);
				
				_menuList.push(lesson);
				category.sublist.push(lesson);
			}
		}
		
		protected function addTextToList(text:XMLList):void
		{
			for(var i:int = 0; i < text.length(); i++)
			{
				_text.addItem(text[i].@id, htmlDecode(text[i]));
			}
		}
		
		public function menuFromScenes(value:Array):void
		{
			var i:int;
			for(i = 0; i < value.length; i++)
			{
				var lesson:Lesson = new Lesson();
				lesson.id = i;
				lesson.label = value[i].name;
				_menuList.push(lesson);
			}
			dispatchEvent(new Event(LessonManager.LESSONS_READY));
		}
		
		public function getTextItem(id:String):String
		{
			return _text.getItem(id);
		}
